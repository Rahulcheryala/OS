...
def main():
    ...
    parser.add_argument('-a', '--asize', default='1k', help='address space size (e.g., 16, 64k, 32m, 1g)', type=str)
    parser.add_argument('-p', '--physmem', default='16k', help='physical memory size (e.g., 16, 64k, 32m, 1g)', type=str)
    parser.add_argument('-n', '--addresses', default=5, help='number of virtual addresses to generate', type=int)
    parser.add_argument('-b', '--b', default='-1', help='value of base register', type=str)
    parser.add_argument('-l', '--l', default='-1', help='value of limit register', type=str)
    parser.add_argument('-c', '--compute', default=False, help='compute answers for me', action='store_true')
    ...
    if psize <= 1 or asize == 0 or psize <= asize:
        print('Error: invalid sizes.')
        sys.exit(1)

    limit = convert(options.l)
    base  = convert(options.b)
